...
function mostrarUsuarios(usuarios) {
    ...
    tbody.innerHTML = usuarios.map(usuario => `
        <tr>
            ...
            <td>
                <button class="btn btn-sm btn-warning mr-1" onclick="window.editarUsuario(${usuario.idUsuario})">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn btn-sm btn-danger" onclick="window.eliminarUsuarioConfirm(${usuario.idUsuario})" ${usuario.activo ? '' : 'disabled'}>
...
async function guardarActividad() {
    try {
        ...
        actividadData.fecha_programada = new Date(actividadData.fecha_programada).toISOString().slice(0, 19).replace('T', ' ');
...
function mostrarInteracciones(interacciones) {
    const tbody = document.getElementById('tablaInteracciones');
    
    if (!interacciones || interacciones.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-center">No hay interacciones registradas.</td></tr>';
        return;
    }
    
    tbody.innerHTML = interacciones.map(interaccion => `
        <tr>
            <td>${interaccion.id_interaccion}</td>
            <td>${interaccion.nombre_cliente || 'N/A'}</td>
            <td><span class="badge badge-info">${interaccion.tipo_interaccion}</span></td>
            <td>${formatearFecha(interaccion.fecha_interaccion)}</td>
            <td>${acortarTexto(interaccion.descripcion, 50)}</td>
            <td>${interaccion.nombre_usuario || 'Sistema'}</td>
            <td>
                <button class="btn btn-sm btn-warning mr-1" onclick="window.editarInteraccion(${interaccion.id_interaccion})">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn btn-sm btn-danger" onclick="window.eliminarInteraccionConfirm(${interaccion.id_interaccion})">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
        </tr>
